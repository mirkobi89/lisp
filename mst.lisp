...
(defun heap-modify-key (heap-id new-key old-key v)
  (if (> new-key old-key)
      nil)
  (let ((i (position old-key heap :key #'first)))
    (setf (aref heap i) (list new-key v))
     (swim i)))
...
(defun heapify (i)
  (let ((r (right i))
	...
	(if (and (< l (heap-size heap-rep))
	    (< (first(aref heap l))(first(aref heap i))))
	    (setf smallest l))
	(if (and (< r (heap-size heap-rep))
	    (< (first(aref heap r)) (first(aref heap smallest))))
	    (setf smallest r))
	(if (/= smallest i)
